...
- **Session persistence**: **shared with the CLI** — same on-disk format and location (`~/.thincoder/sessions/`). Sessions are numbered slots `session.json.N` plus a `session.json.manifest` (slot metadata + active pointer + sessionId). Both ends read/write the same files, so a session created in the CLI appears in VS Code and vice versa. Titles (`title` field) are auto-generated from the first user message on both ends. The legacy `messages/` directory + base64 names + Memento index are abandoned (pre-release, no migration).
