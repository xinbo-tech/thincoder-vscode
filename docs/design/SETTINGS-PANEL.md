...
3. **无法设置 activeProvider**——CLI 的 config.json 有 `activeProvider` 指针（决定默认用哪家），面板里看不到也改不了。当前 VS Code 的"当前 provider"由聊天时的 model prefs 隐式决定，与 CLI 语义脱节。
...
PROVIDERS                                    [+ Add]
┌────────────────────────────────────────────────────┐
│ ● DeepSeek        deepseek-v4-pro      [Key] [−]  │   ← ● = active
│   api.deepseek.com                                │
│ ○ Kimi            kimi-k3              [Key] [−]  │
│   api.moonshot.cn/v1                              │
└────────────────────────────────────────────────────┘
...
- 每行两行式：第一行 label + model + 按钮；第二行 baseURL（dim 小字）。
- 行首 radio 点选 = 设为 activeProvider（写 config.json `activeProvider`，对齐 CLI `/provider use`）。
...
| `setActiveProvider` | `{ name }` | 写 config.json `activeProvider` |
...
- **D3**：activeProvider 的语义对齐 CLI：它决定 `providerFromConfig()` 无参调用时的默认 provider；聊天时用户在模型下拉里选的具体 provider 仍然优先（per-message override，现状不变）。
...
| 文件 | 改动 |
|------|------|
| `webview/settings.js` | provider 区块重排（两行式 + active radio + Key/− 按钮）、[+ Add] 表单视图、Custom 表单加 name/format |
| `webview/settings.css` | 新增样式（active 标记、两行式行、表单视图） |
| `src/extension/settings.mjs` | providerStatus 增加 baseURL/model/isActive 字段；新增 addProvider/removeProvider/setActiveProvider 处理函数 |
| `src/extension/chat-panel.mjs` | 路由三个新消息 |
...
2. 行首 radio 切换 activeProvider，config.json 同步更新，CLI 端 loadConfig 能读到。
