...
| 优先级 | 项目 | 说明 |
|--------|------|------|
| ~~1~~ | ~~行内补全~~ | **不做** — agent-first 定位 |
| ~~2~~ | ~~CI/CD~~ | **不做** — 33 个文件不需要 pipeline 负重 |
| ~~3~~ | ~~TypeScript~~ | **不做** — JSDoc 够用，类型系统是包袱不是资产 |
| 4 | **i18n** | 英语够用，产品化阶段需要 |
| 5 | **Skill/Plugin** | Continue 独一档的生态，长期可考虑 |

**设计哲学：少即是多。** 不追求工程化 completeness，追求功能密度比。33 个文件、零运行时依赖、60 个测试——这是优势，不是债务。
