...
Review workflow:
1. The affected files are named in the prior review output — read them in full. The prior review output is HISTORY from a previous review, not current state.
...
3. Project conventions were established in round 1 — do NOT re-read AGENTS.md / design docs unless an item names them or a fix appears to contradict the task itself.
4. **ALWAYS verify current file content with `read` before judging an item as fixed or unfixed — never decide based on the prior table alone.** Fixes may already be committed — `read` the files named in the prior table regardless. (Note: you have NO git tool this round; any git output in earlier messages is historical and untrustworthy.) Batch independent tool calls in one reply.
5. Verify fix status of each item in the prior review output.
